...
## 4、设置
``` C++
// 新添加设置页的类需要继承自IOptionsPage;
// 新添加设置页部件需要继承自IOptionsPageWidget;
// 设置类别,category唯一
setCategory(Id category)
// 设置显示类别名称
setDisplayCategory(QString displayCategory)
// 设置类别图标
setCategoryIconPath(FilePath categoryIconPath)
// 设置设置页部件的id,id唯一
setId(Id id);
// 设置设置页部件的显示名称
setDisplayName(QString displayName)
// 设置小部件创建器，widgetCreator继承至IOptionsPageWidget
setWidgetCreator(WidgetCreator widgetCreator)
// 实现SettingsPage对象即可添加
...
auto dropSupport = new DropSupport(mainWidget, [](QDropEvent *event, DropSupport *) {
    return event->source() == nullptr; // only accept drops from the "outside" (e.g. file manager)
});
QObject::connect(dropSupport,&DropSupport::filesDropped,this,[this](const QList<DropSupport::FileSpec> &files){
    for (DropSupport::FileSpec file : files) {
        qDebug()<<"filePath="<<file.filePath.toString();
    }
});
```
## 9、设置活动页面
